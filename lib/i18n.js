...
class I18n {
  ...
  loadLocales(directory) {
    ...
    files.forEach((fileName) => {
      ...
      if (extension === ".yaml" || extension === ".yml") {
        ...
        const data = fs.readFileSync(path.resolve(directory, fileName));
  ...
  middleware() {
    return (ctx, next) => {
      ...
      return next().then(() => {
        if (session) {
          session.language_code = ctx.i18n.locale();
        }
      });
    };
  }
...
I18n.match = function (resourceKey, templateData) {
  return (text, ctx) =>
    text && ctx && ctx.i18n && text === ctx.i18n.t(resourceKey, templateData)
      ? [text]
      : null;
};
